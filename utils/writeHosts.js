...
async function writeHosts(content, isAppendFile = false) {
    ...
    // 已经添加过了，并且是追加hosts
    if (curHosts.includes(COMMENT_START) && isAppendFile) {
        writeContent = `${top}\n${COMMENT_START}\n${toolContent}\n${content}\n${COMMENT_END}\n${bottom}\n`
    } else if(curHosts.includes(COMMENT_START) && !isAppendFile) {
        writeContent = `${top}\n${COMMENT_START}\n${content}\n${COMMENT_END}\n${bottom}\n`
    } else {
